        ...
        connect_arduino.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
             if(connect_arduino.getText()=="CONNECT") {
                ...
                connectService.findSerialPortDevice();
                 connect_arduino.setText("DISCONNECT");
             }
             else if(connect_arduino.getText()=="DISCONNECT")
             {
                 if(connectService.isSerialPortConnected())
                    connectService.close_port();
                 else
                     Toast.makeText(Splash.this, "Device already disconnected", Toast.LENGTH_SHORT).show();
   ...
   */
    ...
    void updateChart(float y)
    {
      //  data.addEntry(new Entry(x/1000,y),0);
        ...
        Log.i("x_incre",""+x);
        EventBus.getDefault().post(new Message(x/1000000000,y));
        do_it=false;
